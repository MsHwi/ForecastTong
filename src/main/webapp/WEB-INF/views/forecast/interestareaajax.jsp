...
<script type="text/javascript">
	...
	$(document).one("ready", function() {
		$(function() {
			$.getJSON('/overallfc/getfirstlocjson', function(data) {
	...
	$("#locContents1").click(function(e) {
		...
		var url = "/overallfc/selsecondstage?firststage="+value;	
		$('#locContents1').hide();
	...
	$("#locContents2").click(function(e) {
		...
		if(value2 == "upto") {
			...
			var url = "/overallfc/selthirdstage?A_FST_STAGE="+value+"&A_SCND_STAGE="+value2;
			$('#locContents2').hide();
	...
	$('#submit_loc').click(function(e) {
		if ($('#loc_stage3').val().length <= 1 || $('#loc_stage3').val() == "주세요." || $('#loc_stage3').val() == "") {
			...
			alert("저장되었습니다.");
			$("#iarea").attr("action", "/overallfc/addinterestarea");
			$("#iarea").submit();	
			$("#popup").hide();
			location.reload();
			return true; 
...
<body>
	...
	<span style="margin-right:5px;"><button type="button" class="btn btn-block btn-primary" id="locfsct_show" style="width:100px;height:30px;float:right;margin-left:10px" onclick="showForecast()">예보보기</button></span>
	<span><a href="" data-toggle="modal" data-target="#modal-selstage"><button type="button" class="btn btn-block btn-primary" id="loc_menu" style="width:130px;height:30px;float:right;">관심지역 설정</button></a></span>
	<select class="form-control" id="interestarea_sel" style="margin-top:15px;margin-bottom:10px;">
		<c:forEach var="list" items="${interestAreaList}" varStatus="status">
			<option>${list.getA_FST_STAGE()} ${list.getA_SCND_STAGE()} ${list.getA_THIRD_STAGE()}</option>
		</c:forEach>
	</select>
				...
				<div class="modal-body">
					...
					<div id="popupContents" class="form-group">
						<fieldset class="boardTop">
								<div class="row">
									<div class="col-xs-3"><input type="text" class="form-control" id="loc_stage1"  name ="A_FST_STAGE" readonly="readonly" value="" /> </div>
									<div class="col-xs-3"><input type="text" class="form-control" id="loc_stage2" name ="A_SCND_STAGE" readonly="readonly" value=""  /> </div>
									<div class="col-xs-3"><input type="text" class="form-control" id="loc_stage3" name ="A_THIRD_STAGE" readonly="readonly"  value="" /> </div>
									<input type="hidden" name ="U_ID"  value="${sessionScope.login.getU_ID()}" />
