...
.fsnjisu_container .fsnjisutab_content ul {
	width: 100%;
	margin: 0px;
	padding: 0px;
}

.fsnjisu_container .fsnjisutab_content  ul li {
	padding: 0px;
	list-style: none;
}
...
window.onload = function() {
	 ...
	 //alert("fsnjisu.. ready()222 v_areano1 : " + v_areano1);
...
$(function(){
   ...
   //var selectValue = document.getElementById('favLocalSelect').value;
   //alert("selectValue : " + selectValue)
   //$("#favLocalSelect1").onchange(selectValue);
   
   // value 값으로 선택
  //$("#select_id").val("1").prop("selected", true);
  //OR option 순서값으로 선택
  //$("#favLocalSelect1 option:eq(1)").prop("selected", true);
//지정된 index 값으로 select 하기
 /* $("#favLocalSelect1 option:eq(2)").attr("selected", "selected");
  
   //var selectValue = $("#favLocalSelect1 option:selected").val();
   var selectValue = $("#favLocalSelect1 option:selected").text();
   alert("selectValue : " + selectValue);  
   */  
  ...
  function localJisuSelService_call(v_areano1){
	   //alert("fsnjisu.jsp localJisuSelService_call().... v_areano : " + v_areano1); 
    
  		var url = "/lifefc/FsnJisuSelService.do?areano=" + v_areano1 +"&tabno=1";
...
<script type="text/javascript">
	$(function() {
		// 오늘/내일/모레 탭에 대한 처리
		$(".fsnjisutab_content").hide();
		$(".fsnjisutab_content:first").show();
...
<%-- 			var url = "./UtryJisuSelService.do?areano=<%=v_areano%>&tabno="+tabno; --%>
			var url = "/lifefc/FsnJisuSelService.do?areano="+v_areano1+"&tabno="+tabno;
	...
	function winopen() {
		//alert("winopen");
		var url = "/lifefc/AllJijumJisuSelect.do?jisuname=식중독";//"./lifefc/loc_ajax.jsp";
		var winName = "post";
...
<br/>
	<div id="fsnjisu_container">
		<ul class="tabs">
			<li class="active" rel="mainDiv2">오늘</li>
			<li rel="tab2">내일</li>
			<li rel="tab3">모레</li>
		</ul>
		<div class="fsnjisu_container">
			<div id="mainDiv2" class="fsnjisutab_content" name ="mainDiv2">   
				<iframe src="/lifefc/FsnJisuSelService.do?areano=<%=v_areano%>&tabno=1"></iframe>
			</div>
			<div id="tab2" class="fsnjisutab_content" name ="tab2">

			</div>
			<div id="tab3" class="fsnjisutab_content" name ="tab3">
			</div>			
		</div>
	</div>
